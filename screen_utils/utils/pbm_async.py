...
async def read_header(instream):
...
async def read_data(instream, width, height, format, offset=-1):
    r'''Read image data
    :param: instream: stream that support seek, tell, read
            width: image width
            height: image height
...
async def read_image(instream):
    ...
    '''
    width, height, mg, _ = await read_header(instream)
    data = await read_data(instream, width, height, mg)
    return (width, height, mg, data)

async def make_image(outstream, width, height, data, comment="made with bpm.py"):
    ...
    while index < length:
        await asyncio.sleep(0)
        outstream.write(data[index:index+128])
        index += 128
    await asyncio.sleep(0)
    outstream.write("\n".encode())
    size = outstream.tell()
